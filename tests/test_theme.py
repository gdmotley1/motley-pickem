...
def test_there_is_only_one_rank_style():
    # Comments stripped first: the .aprank rule's own note names both old classes to
    # explain why they went, and matching that would fail the test forever.
    css = re.sub(r"/\*.*?\*/", "", read(APP_CSS), flags=re.S)
    for gone in (".tpick__rank", ".mu__rank"):
